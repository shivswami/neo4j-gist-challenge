...
[source,cypher]
----
MATCH (javaDev:Person {name:"Java Developer"}), (javaDev)-[:UNDERSTANDS]->(tech)
...
[source,cypher]
----
MATCH (sdn:Tech {name:"Spring Data Neo4j"})-[:REQUIRES*0..]->(technologyOrConcept)<-[teaches:TEACHES]-(resource),
      (javaDev:Person {name:"Java Developer"}) 
